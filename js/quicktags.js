    ...
    if (!edCheckOpenTags(c)) {
        ...
        if (matches) {
            ...
            if (refs.length > 1) {
                ...
                jQuery('#reflinkwrap').slideDown('fast', function() {
                    jQuery('#selectmenu').focus();
                });
...
function edMakeList(d, c, type) {
    var listsel = getSelectionCoord(d),
        newend = listsel.end;
    if (document.selection) {
        d.focus();
        var e = document.selection.createRange();
...
(function($) {
        $.mdqt_init = function(options) {
            var $content = $('#content'),
                ...
                $container = $('#editorcontainer').length > 0 ? $('#editorcontainer') : $('#wp-content-editor-container');
            ...
            $('<input type="button" id="previewbutton" title="Preview"/>').click(function(e) {
                ...
                if (!$(this).hasClass('activeutil')) {
                    ...
                    $container.css({
                        height: ($('div.postarea').height() - 52) + 'px'
                    });
            ...
            $('<input type="button" id="fullscreenbutton" title="Full Screen Editor"/>').click(function(e) {
                    ...
                    if (!$(this).hasClass('activeutil')) {
                        ...
                        $('<div id="fullscreenoverlay" />')
                            .css({
                                'display': 'none',
                                'background': options.fullscreenbg,
                                'opacity': options.fullscreenbgopacity
                            })
                            ...
                            height: '100%'
                                ...
                                $(this).addClass('supermarkdowneditoraway').css({
                                    ...
                                    'margin-left': '-' + marginleft + 'px'
                                        ...
                                        $('#content,#preview').css({
                                                height: ($('div.postarea').height() - 95) + 'px');
                                        });
                                ...
                                $('#preview').css({
                                    height: ($('div.postarea').height() - 136) + 'px'
                                });
                            });
                        } else {
                            $(this).removeClass('activeutil');
                            $('div.postarea').fadeOut('fast', function() {
                                $(this).removeClass('supermarkdowneditoraway').css({
                                    height: 'auto',
                                    width: 'auto',
                                    marginLeft: 'auto'
                                });
                                $('#fullscreenoverlay').fadeOut('fast', function() {
                                    $(this).remove();
                                    $('div.postarea').fadeIn('fast', function() {
                ...
                $('<form id="urlpaste" title="Paste text with one or more urls in it..."/>').css({
                    display: 'none',
                    textAlign: 'center'
                }).append($('<textarea />')).appendTo($('#ed_toolbar')); $("#urlpaste").dialog({
                    autoOpen: false,
                    height: 400,
                    width: 500,
                    modal: true,
                ...
                if (options.showtooltips)
                    $('#ed_toolbar input, #helpbutton').tipsy({
                        fade: true,
                        gravity: 's'
                    });
                if (options.font)
                ...
                }); takeSnapshot($content.val(), getSelorCaret(content)); $('#mdqt_loader').fadeOut('fast');
            }
        })(jQuery);
